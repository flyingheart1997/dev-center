...
### A. Registration & Intent-Based Onboarding
* **Unified Registration (`/register`)**: Email -> Password (with `checkEmailExists` validation) OR Social Auth (Google/GitHub). No intent selection happens before registration.
* **Email Verification Gate**: All email/password sign-ups must be verified before proceeding. Unverified users are blocked at `/verify-email`. Social logins are auto-verified.
* **Post-Registration Intent (`/onboarding`)**: Once registered and verified, users without a workspace are routed to `/onboarding` to choose their path: "Candidate" or "Organization".
* **Atomic Organization Setup (`/setup-org`)**: Org Founders set up company details. A Prisma `$transaction` atomically creates the `Organization`, default `BusinessUnit`, default `Branch`, default `Department`, and links the user as an `Employee` (Role: `OWNER`).
* **Tokenized Employee Invites**: Team members and employees join strictly via tokenized invitation links generated by Admins.
* **Cross-Workspace Protection**: Strict proxy routing prevents Candidates from accessing the employer `/dashboard` (or `/setup-org`), and prevents Organization Employees from accessing the `/candidate` portal.
...
│   │   ├── login/page.tsx
│   │   ├── register/page.tsx # Unified Registration (Email & Password / Social)
│   │   ├── onboarding/page.tsx # Intent Step: Candidate vs Organization
